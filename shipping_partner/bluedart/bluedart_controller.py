...
bluedart_router = APIRouter(tags=["Bluedart"])

security = HTTPBearer()


@bluedart_router.post(
    "/webhook/courier/bluedart/track-order",
...
async def tracking_webhook(request: Request):
    ...
    response: GenericResponseModel = Bluedart.tracking_webhook(track_req=track_req)
    return build_api_response(response)


@bluedart_router.post(
    "/dev/webhook/courier/bluedart/track-order",
    status_code=http.HTTPStatus.CREATED,
    response_model=GenericResponseModel,
)
async def tracking_webhook(request: Request):
