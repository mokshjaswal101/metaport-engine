...
shiprocket_router = APIRouter(tags=["shiprocket"])

security = HTTPBearer()


@shiprocket_router.post(
    "/webhook/courier/shprckt/track-order",
...
async def tracking_webhook(request: Request):
    ...
    response: GenericResponseModel = Shiprocket.tracking_webhook(track_req=track_req)
    return build_api_response(response)


@shiprocket_router.post(
    "/webhook/courier/shprckt2/track-order",
    status_code=http.HTTPStatus.CREATED,
    response_model=GenericResponseModel,
)
async def tracking_webhook(request: Request):
