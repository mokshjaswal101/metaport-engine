...
class QcService:
    ...
    @staticmethod
    async def get_qc() -> GenericResponseModel:
        try:
            async with get_db_session() as db:
                user_data = context_user_data.get()
                client_id = user_data.client_id

                # Fetch all qc rows for client
                result = await db.execute(
                    select(Qc).where(Qc.client_id == client_id, Qc.is_deleted == False)
                )
                qc_items = result.scalars().all()
                ...
                if not qc_items:
                    return GenericResponseModel(
                        status=True,
                        status_code=http.HTTPStatus.OK,
                        message="No QC data found",
                        data=[],
                    )

                # ----------- GROUP BY category + reason_name ------------
                grouped_data = {}
                for item in qc_items:
                    key = (item.category, item.reason_name)
                    ...
                    #     {
                    #         "parameters_name": item.parameters_name,
                    #         "parameters_value": item.parameters_value,
                    #         "is_mandatory": item.is_mandatory,
                    #     }
                    # )
                ...
                return GenericResponseModel(
                    ...
                    status_code=http.HTTPStatus.OK,
                    message="QC list fetched successfully",
                    data=jsonable_encoder(final_data),
                )
